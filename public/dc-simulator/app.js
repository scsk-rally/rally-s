...
function calculate(){
 ...
 if(!valid){for(const id of ['futureValue','principalOnlyValue','matchingEffect','companyPrincipal','personalPrincipal','gain'])$(id).textContent='—';return;}
 ...
 const companyPrincipal=company*months,personalPrincipal=personal*months,principalOnly=balance+companyPrincipal+personalPrincipal,gain=future-principalOnly;
 $('futureValue').textContent=man(future);$('principalOnlyValue').textContent=man(principalOnly);$('matchingEffect').textContent=(effect>=0?'+':'−')+man(Math.abs(effect))+'万円';$('companyPrincipal').textContent=man(companyPrincipal)+'万円';$('personalPrincipal').textContent=man(personalPrincipal)+'万円';$('gain').textContent=(gain>=0?'+':'−')+man(Math.abs(gain))+'万円';
}
